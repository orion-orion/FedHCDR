...
class Server(object):
    def __init__(self, args, init_global_params=None):
        self.args = args
        self.global_params = init_global_params
        if args.method == "FedHCDR":
            self.global_reps = [None]
        elif args.method in ["FedPriCDR", "FedP2FCDR"]:
            self.global_reps = [None, None]  # U_mlp_g, U_mf_g
        elif args.method == "FedPPDM":
            self.global_reps = [None, None]  # U_mu_g, U_sigma_g
    ...
    def aggregate_params(self, clients, random_cids):
        ...
        for branch_idx in range(num_branchs):
            ...
            for c_id in random_cids:
                ...
                current_client_params = clients[c_id].get_params_shared()[
                    branch_idx]
    ...
    def aggregate_reps(self, clients, random_cids):
        """Sums up representations of users shared by all active
        ...
        Args:
            ...
            random_cids: Randomly selected client ID in each training round.
        """
        # Record the user representations aggregation results of each branch
        # separately
        num_branchs = len(self.global_reps)
        for branch_idx in range(num_branchs):
            client_reps_sum = None
            for c_id in random_cids:
                # Obtain current client's user representations
                ...
                if client_reps_sum is None:
                    ...
                    client_reps_sum += clients[c_id].train_weight * \
                        current_client_reps
            self.global_reps[branch_idx] = client_reps_sum
