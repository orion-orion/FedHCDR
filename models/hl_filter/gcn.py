...
class GCNLayer(nn.Module):
    ...
    def __init__(self, num_features, num_hidden, dropout, alpha, args,
        ...
        self.gc1 = GraphConvolution(num_features, num_hidden, args, mode=mode)
...
class GraphConvolution(Module):
    def __init__(self, in_features, out_features, args, bias=True, mode="low"):
    ...
    def forward(self, input, adj):
        ...
        I = torch.ones(adj.shape[0], 1).to(self.device)
        if self.mode == "low":
            output = torch.spmm(adj, support) \
                + (1 + self.beta) * I * support
        else:
            output = torch.spmm(-adj, support) \
                + (1 - self.beta) * I * support
        if self.bias is not None:
